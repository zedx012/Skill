...
function CourseCard({ course, index, completedLessonIds, onOpenCourse }: CourseCardProps) {
...
export function SkillPaths({ courses, progress, onOpenCourse }: SkillPathsProps) {
    ...
    <section className="animate-fade-up" style={{ animationDelay: '160ms' }}>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((course, i) => (
          <CourseCard key={course.id} course={course} index={i} completedLessonIds={completedLessonIds} onOpenCourse={onOpenCourse} />
        ))}
      </div>
    </section>
  );
