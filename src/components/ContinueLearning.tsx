...
interface ContinueLearningProps {
  ...
  onOpenLesson: (lessonId: string, courseId: string) => void;
}

export function ContinueLearning({ course, lessons, progress, onOpenCourse, onOpenLesson }: ContinueLearningProps) {
  ...
  return (
    <section className="animate-fade-up" style={{ animationDelay: '80ms' }}>
      <div className="flex items-baseline justify-between mb-3">
        ...
        <button onClick={() => course && onOpenCourse(course.id)} className="focus-ring text-[11px] font-medium text-ink-500 hover:text-ink-900 transition-colors flex items-center gap-0.5">
          ...
          <div className="relative p-6 lg:p-8 overflow-hidden">
            ...
            <div className="relative">
              ...
              <button
                onClick={() => {
                  if (currentLesson) {
                    onOpenLesson(currentLesson.id, course.id);
                  } else {
                    onOpenCourse(course.id);
                  }
                }}
                className="focus-ring group inline-flex items-center gap-2.5 px-5 py-3 bg-ink-900 text-paper-100 rounded-lg font-medium text-[13px] hover:bg-ink-800 transition-all duration-200 shadow-lifted hover:scale-[1.01] active:scale-[0.99]"
              >
              ...
              {lessons.map((lesson) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => onOpenLesson(lesson.id, course.id)}
